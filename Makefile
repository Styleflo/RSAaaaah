...
CC = clang
CFLAGS = -I/opt/homebrew/opt/openssl/include -Wall -Wextra -pthread
...
# Lancer le serveur avec un port spécifique (par défaut 4433)
...
# Lancer le client avec une IP et un port (par défaut 127.0.0.1:4433)
...
# Lancer le client avec une IP et un port spécifiques
